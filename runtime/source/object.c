...
void free_object(Object* obj) {
  ...
  switch (obj->type) {
    case O_ERROR: {
      obj_size = sizeof(ObjectError) + sizeof(Symbol) * OBJ_ERR(obj)->size;
      break;
    }

    case O_VECTOR: {
      obj_size = sizeof(ObjectVector) + sizeof(double) * obj_vec_size(OBJ_VEC(obj));
      break;
    }
    ...
    case O_DATA: {
      obj_size = sizeof(ObjectData) + sizeof(Value) * OBJ_DAT(obj)->size;
      break;
    }

    case O_SET: {
      obj_size = sizeof(ObjectSet) + sizeof(Value) * OBJ_SET(obj)->cap;
      break;
    }

    case O_DICTIONARY: {
      obj_size = sizeof(ObjectDictionary) + sizeof(ObjectDictionaryEntry) * OBJ_DCT(obj)->cap;
      break;
    }
...
bool equal_objects(Object* a, Object* b) {
  ...
  switch (a->type) {
    case O_ERROR: return obj_err_equal(OBJ_ERR(a), OBJ_ERR(b));
    case O_VECTOR: return obj_vec_equal(OBJ_VEC(a), OBJ_VEC(b));
    case O_STRING: return obj_str_equal(OBJ_STR(a), OBJ_STR(b));
    case O_DATA: return obj_dat_equal(OBJ_DAT(a), OBJ_DAT(b));
    case O_SET: return obj_set_equal(OBJ_SET(a), OBJ_SET(b));
...
void print_object(Object* obj) {
  switch (obj->type) {
    case O_ERROR: printf("%s!'%s'", OBJ_ERR(obj)->kind.str, OBJ_ERR(obj)->message); break;
    case O_VECTOR: printf("<vec>"); break;
    case O_STRING: printf("'%s'", OBJ_STR(obj)->data); break;
    case O_DATA: printf("[data]"); break;
    case O_SET: printf("#{set}"); break;
...
ObjectString* obj_str_from_raw(const char* s) {
  ...
  ObjectString* obj =
    ...
    (ObjectString*)alloc_object(O_STRING, sizeof(ObjectString) + str_size + sizeof(char));
...
ObjectArray* obj_arr_with_size(size_t n) {
  ObjectArray* obj = (ObjectArray*)alloc_object(O_ARRAY, sizeof(ObjectArray) + sizeof(Value) * n);
...
ObjectArray* obj_arr_append(ObjectArray* arr, Value value) {
  ObjectArray* obj =
    (ObjectArray*)alloc_object(O_ARRAY, sizeof(ObjectArray) + sizeof(Value) * (arr->size + 1));
...
ObjectDictionary* obj_dct_with_cap(size_t n) {
  ...
  ObjectDictionary* obj = (ObjectDictionary*)alloc_object(O_DICTIONARY, sizeof(ObjectDictionary));
...
void obj_dct_insert(ObjectDictionary* obj, Value key, Value value) {
  ...
  if ((double)obj->len + 1 > obj->cap * DICTIONARY_LOAD_FACTOR) {
...
ObjectArray* obj_dct_keys(ObjectDictionary* obj) {
  ...
  for (size_t i = 0; i < obj->cap; i++) {
    if (obj_dct_entry_occupied(&obj->entries[i])) { keys->vals[i] = obj->entries->key; }
  }
...
ObjectArray* obj_dct_values(ObjectDictionary* obj) {
  ...
  for (size_t i = 0; i < obj->cap; i++) {
    if (obj_dct_entry_occupied(&obj->entries[i])) { values->vals[i] = obj->entries->value; }
  }
...
ObjectClosure* obj_clj_from_fct(ObjectFunction* fct) {
  ObjectClosure* obj = (ObjectClosure*)alloc_object(O_CLOSURE, sizeof(ObjectClosure));
...
ObjectHeapval* obj_hpv_promote(Value val) {
  ObjectHeapval* obj = (ObjectHeapval*)alloc_object(O_HEAPVAL, sizeof(ObjectHeapval));
