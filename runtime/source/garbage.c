...
static void mark_object(Object *obj) {
  ...
  case O_DICTIONARY: {
    ...
    for (size_t i = 0; i < dict->len; i++) {
      ...
      if (entry->key.type == T_OBJ)
        mark_object(entry->key.as.object);
      if (entry->value.type == T_OBJ)
        mark_object(entry->value.as.object);
    }
  ...
  case O_HEAPVAL: {
    ...
    if (upv->val.type == T_OBJ)
      mark_object(upv->val.as.object);
    break;
  }
  }
}
