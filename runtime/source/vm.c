...
static inline Value unary_op(VM *vm, unary_op_fct f) { return f(vm, POP()); }
...
static inline Value read_int_(VM *vm) {
  union {
    uint64_t value;
    struct {
      uint32_t second;
  ...
  read.parts.first = ARG4;
  read.parts.second = ARG4;

  return INT_VAL(read.value);
}

static inline Value read_real_(VM *vm) {
  union {
    double value;
    struct {
      uint32_t second;
...
static inline Value negate_(VM *vm, Value a) {
  switch (a.type) {
  case T_INT:
    return INT_VAL(-a.as.integer);
  case T_REAL:
    return REAL_VAL(-a.as.real);
  default:
...
static inline Value not_(VM *vm, Value a) { return BOOL_VAL(falsy(a)); }
...
static inline Value add_(VM *vm, Value a, Value b) {
  ...
  case TUP(T_REAL, T_REAL):
    return REAL_VAL(a.as.real + b.as.real);

  case TUP(T_INT, T_REAL):
    return REAL_VAL(a.as.integer + b.as.real);
  case TUP(T_REAL, T_INT):
    return REAL_VAL(a.as.real + b.as.integer);
  ...
  case TUP(T_OBJ, T_OBJ): {
    if (IS_OBJ_STR(a) && IS_OBJ_STR(b)) {
      Object *obj = (Object *)obj_str_concat(OBJ_STR(a.as.object)->data,
                                             OBJ_STR(b.as.object)->data);
...
static inline Value divide_(VM *vm, Value a, Value b) {
  ...
  case TUP(T_REAL, T_REAL):
    return REAL_VAL(a.as.real / b.as.real);
  case TUP(T_INT, T_REAL):
    return REAL_VAL((double)a.as.integer * b.as.real);
  case TUP(T_REAL, T_INT):
    return REAL_VAL(a.as.real / (double)b.as.integer);
  ...
  case TUP(T_OBJ, T_STR): {
    ...
    Object *obj = (Object *)obj_str_concat_sep(OBJ_STR(a.as.object)->data,
                                               PATH_SEPARATOR, b.as.string);
...
static inline Value modulo_(VM *vm, Value a, Value b) {
  ...
  case TUP(T_INT, T_INT):
    return INT_VAL(a.as.integer % b.as.integer);
  case TUP(T_REAL, T_REAL):
    return REAL_VAL(fmod(a.as.real, b.as.real));
  case TUP(T_INT, T_REAL):
    return REAL_VAL(fmod(a.as.integer, b.as.real));
  case TUP(T_REAL, T_INT):
    return REAL_VAL(fmod(a.as.real, b.as.integer));
  default:
    SETERR(STATUS_INVTYP);
    return VOID_VAL;
  }
...
static inline Value rounddiv_(VM *vm, Value a, Value b) {
  ...
  case TUP(T_INT, T_INT):
    return INT_VAL(a.as.integer / b.as.integer);
  case TUP(T_REAL, T_REAL):
    return INT_VAL(a.as.real / b.as.real);
  case TUP(T_INT, T_REAL):
    return INT_VAL(a.as.integer / b.as.real);
  case TUP(T_REAL, T_INT):
    return INT_VAL(a.as.real / b.as.integer);
  default:
    SETERR(STATUS_INVTYP);
    return VOID_VAL;
  }
...
static inline Value power_(VM *vm, Value a, Value b) {
  switch (TUP(a.type, b.type)) {
  case TUP(T_INT, T_INT):
    return INT_VAL(pow(a.as.integer, b.as.integer));
  case TUP(T_REAL, T_REAL):
    return REAL_VAL(pow(a.as.real, b.as.real));
  case TUP(T_INT, T_REAL):
    return REAL_VAL(pow(a.as.integer, b.as.real));
  case TUP(T_REAL, T_INT):
    return REAL_VAL(pow(a.as.real, b.as.integer));
  default:
    SETERR(STATUS_INVTYP);
    return VOID_VAL;
  }
...
static inline Value append_(VM *vm, Value a, Value b) {
  // Handle arrays
...
#define NUM_ORDERING_OP(op)                                                    \
  switch (TUP(a.type, b.type)) {                                               \
  case TUP(T_INT, T_INT):                                                      \
    return BOOL_VAL(a.as.integer op b.as.integer);                             \
  case TUP(T_REAL, T_REAL):                                                    \
    return BOOL_VAL(a.as.real op b.as.real);                                   \
  case TUP(T_INT, T_REAL):                                                     \
    return BOOL_VAL(a.as.integer op b.as.real);                                \
  case TUP(T_REAL, T_INT):                                                     \
    return BOOL_VAL(a.as.real op b.as.integer);                                \
  case TUP(T_CHAR, T_CHAR):                                                    \
    return BOOL_VAL(a.as.charac op b.as.charac);                               \
  default:                                                                     \
    SETERR(STATUS_INVTYP);                                                     \
    return VOID_VAL;                                                           \
  }

static inline Value greater_(VM *vm, Value a, Value b) { NUM_ORDERING_OP(>); }

static inline Value greater_eq_(VM *vm, Value a, Value b) {
  NUM_ORDERING_OP(>=);
}

static inline Value less_(VM *vm, Value a, Value b) { NUM_ORDERING_OP(<); }

static inline Value less_eq_(VM *vm, Value a, Value b) { NUM_ORDERING_OP(<=); }
...
static inline void call_(VM *vm, uint8_t argc) {
  ...
  if (IS_NTV_FUN(value)) {
    NativeFn fct = (NativeFn)value.as.native;

    // Pop all arguments off the stack
    vm->stk.vtop -= argc;

    Value ret = VOID_VAL;
    vm->sts = fct(argc, vm->stk.vtop, &ret);
...
void vm_run(VM *vm) {
  ...
  do {
    switch (NEXT) {
      ...
      // Frame operations (useful for TCO)
      ...
      // Load immidiate values
      CASE(VM_INT, PUSH(FUNC(read_int_)));
      CASE(VM_REA, PUSH(FUNC(read_real_)));
      CASE(VM_CHR, PUSH(CHAR_VAL(ARG4)));
      ...
      // Load the value assigned to a symbol
      CASE(VM_GET, LOAD_SYMBOL(ARG1));
      CASE(VM_GET2, LOAD_SYMBOL(ARG2));
      CASE(VM_GET3, LOAD_SYMBOL(ARG3));
      CASE(VM_GET4, LOAD_SYMBOL(ARG4));
      ...
      CASE(VM_GTE, BOP(greater_eq_));
      CASE(VM_LT, BOP(less_));
      CASE(VM_LTE, BOP(less_eq_));
    }
  } while (true);
}

void vm_load(VM *vm, Program *prog, bool debug) {
  vm->prg = prog;
  vm->ip = prog->bytes;
  vm->sts = STATUS_OK;
  vm->dbg = debug;
...
void vm_bind(VM *vm, const char *name, NativeFn fct) {
  // Find symbol in symtable
  Symbol *symbol = symtable_find(&vm->prg->stb, name);

  // Function is unused in program.
  if (!symbol)
    return;

  // Add function call to environment
  env_set(&vm->env, *symbol, NTV_FUN_VAL(fct));
}
