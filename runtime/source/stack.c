...
void reset_stk(Stack *stk) { init_stk(stk); }

Value stk_top(Stack *stk) { return *(stk->vtop - 1); }
...
Frame *stk_frame(Stack *stk) { return stk->ftop - 1; }
...
Value stk_get(Stack *stk, size_t i) { return *(stk_frame(stk)->bp + i); }
