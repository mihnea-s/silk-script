...
#define STACK_MAX_ALLOC_SIZE 32
#define STACK_MIN_ALLOC_SIZE 8
...
void *memory(void *ptr, size_t old_sz, size_t new_sz) {
  ...
  // is over 32 bytes
  if (top == bottom || new_sz > STACK_MAX_ALLOC_SIZE) {
  ...
  void *new_ptr = top;
  top += MAX(new_sz, STACK_MIN_ALLOC_SIZE);
