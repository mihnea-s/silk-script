...
static const char *const header = "SILKEXE";
static const char *const footer = "SILKEND";

uint32_t checksum(const Program *prog) {
...
#define DOUBLE_FRAC_DIGITS 10E+10
...
static uint8_t read_u8(FILE *f) { return fgetc(f); }
...
static double read_dbl(FILE *f) {
  ...
  return integral + (fraction / DOUBLE_FRAC_DIGITS);
}
...
static void read_value(Value *x, FILE *f, const char **err) {
  ...
  case T_NTV_FUN: // fallthrough
  case T_NTV_PTR: {
    SET_ERR("silk executable contains pointers");
    break;
  }
...
static void write_dbl(double d, FILE *f) {
  double ingr = 0, frac = 0;
  frac = modf(d, &ingr);

  uint32_t integral = ingr;
  uint32_t fraction = frac * DOUBLE_FRAC_DIGITS;
...
static void write_value(Value v, FILE *f) {
  ...
  switch (v.type) {
  case T_BOOL:
    return write_u8(v.as.boolean, f);
  case T_INT:
    return write_i64(v.as.integer, f);
  case T_REAL:
    return write_dbl(v.as.real, f);
  case T_CHAR:
    return write_chr(v.as.charac, f);
  case T_STR:
    return write_str(v.as.string, f);
  case T_OBJ:
    return write_obj(v.as.object, f);
  default:
    return;
  }
}

static void write_symbol(Symbol sy, FILE *f) { write_str(sy.str, f); }

void write_file(const char *file, const Program *prog, const char **err) {
  FILE *f = fopen(file, "w");
