...
private:
  ...
  auto load_constant(char32_t) -> void;
  auto load_constant(std::string_view) -> void;
  auto load_identifier_val(const ASTNode &, std::string_view) -> void;
  auto load_identifier_ref(const ASTNode &, std::string_view) -> void;
  ...
  auto evaluate(ASTNode &, DictionaryLiteral &) -> void override;
...
public:
  ...
  auto dry_compile(AST &) noexcept -> void;
